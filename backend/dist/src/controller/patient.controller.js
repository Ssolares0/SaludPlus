...
exports.getAndUpdateProfile = exports.getAllPatientAppointments = exports.cancelAppointment = exports.activesAppointment = exports.createAppointment = exports.scheduleMedic = exports.findMedic = exports.doctorsAvailables = void 0;
...
const slotsPacient_1 = require("../helpers/slotsPacient");
const Patient_entity_1 = require("../models/Patient.entity");
const User_entity_1 = require("../models/User.entity");
const doctorsAvailables = async (req, res) => {
    try {
        // Consulta para obtener los doctores con role = 2 y approved = true
        const doctors = await Postgres_1.AppDataSource.manager
        ...
        const formattedDoctors = doctors.map(doctor => {
            var _a, _b, _c;
            const especialidades = ((_c = (_b = (_a = doctor.person) === null || _a === void 0 ? void 0 : _a.employee) === null || _b === void 0 ? void 0 : _b.specialty) === null || _c === void 0 ? void 0 : _c.map(es => { var _a; return (_a = es.specialty) === null || _a === void 0 ? void 0 : _a.name; })) || [];
            return {
                id: doctor.id,
...
exports.doctorsAvailables = doctorsAvailables;
const findMedic = async (req, res) => {
    try {
        ...
        const doctors = await Postgres_1.AppDataSource.manager
            .createQueryBuilder(Employe_entity_1.Employee, 'employee')
...
exports.findMedic = findMedic;
const scheduleMedic = async (req, res) => {
    ...
    const doctorSchedule = await Postgres_1.AppDataSource.manager
        .createQueryBuilder(DoctorSchedule_entity_1.DoctorSchedule, 'schedule')
    ...
    const appointments = await Postgres_1.AppDataSource.manager
        .createQueryBuilder(Appointments_entity_1.Appointment, 'appointment')
    ...
    const timeSlots = (0, slotsPacient_1.generateTimeSlots)(daySchedule.start_time, daySchedule.end_time, 60, // Intervalo de 60 minutos
    selectedDate);
    ...
    const correctDay = (0, slotsPacient_1.getDayName)(dayOfWeek);
    res.status(200).json({
        error: false,
        data: {
            doctorSchedule: {
...
exports.scheduleMedic = scheduleMedic;
const createAppointment = async (req, res) => {
    try {
        ...
        const existingAppointment = await Postgres_1.AppDataSource.manager
            .createQueryBuilder(Appointments_entity_1.Appointment, 'appointment')
            .where('appointment.employee_id = :doctorId', { doctorId })
        ...
        const doctor = await Postgres_1.AppDataSource.manager.findOne(Employe_entity_1.Employee, {
            where: { id: doctorId },
            relations: ['person']
        });
        if (!doctor) {
        ...
        const result = await Postgres_1.AppDataSource.createQueryBuilder()
            .insert()
            .into(Appointments_entity_1.Appointment)
            .values({
...
const activesAppointment = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
            res.status(400).json({
        ...
        const appointment = await Postgres_1.AppDataSource.manager
            .createQueryBuilder(Appointments_entity_1.Appointment, 'appointment')
            .select([
...
exports.activesAppointment = activesAppointment;
const cancelAppointment = async (req, res) => {
    try {
        ...
        await Postgres_1.AppDataSource.manager.save(Appointments_entity_1.Appointment, appointment);
        res.status(200).json({
            error: false,
...
const getAndUpdateProfile = async (req, res) => {
    try {
        ...
        if (firstName)
            patient.person.first_name = firstName;
        if (lastName)
            patient.person.last_name = lastName;
        if (phone)
            patient.person.phone = phone;
        if (address)
            patient.person.address = address;
        if (birth_date)
            patient.person.birth_date = new Date(birth_date);
        if (gender)
            patient.person.gender = gender;
        if (photoUrl)
            patient.person.photo = photoUrl;
        // Guardar los cambios
        await Postgres_1.AppDataSource.manager.save(patient.person);
        res.status(200).json({
            error: false,
...
exports.getAndUpdateProfile = getAndUpdateProfile;
