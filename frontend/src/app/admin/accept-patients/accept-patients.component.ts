...
import { PendingPatientsResponse } from '../models/admin.models';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, LucideAngularModule, FormsModule, HttpClientModule, SafeImagePipe, ModalComponent],
...
export class AcceptPatientsComponent implements OnInit {
    ...
    this.adminService.acceptUser(patient.id).subscribe({
      next: (response) => {
        if (response && response.success) {
          ...
          this.pendingPatients = this.pendingPatients.filter(p => p.id !== patient.id);
  ...
  acceptPatient(patient: PendingPatientsResponse): void {
    this.processingPatient = {
      id: patient.id,
      name: `${patient.firstame} ${patient.lastName}`
    };
  ...
  rejectPatient(patient: PendingPatientsResponse): void {
    this.showModal(
      'warning',
      'Función no Implementada',
      `La función para rechazar pacientes aún no está implementada. Esta acción rechazaría a ${patient.firstame} ${patient.lastName}.`
    );
  }
